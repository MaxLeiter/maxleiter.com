...
/**
 * Standalone platform check: `pnpm test`, which is `bun run` and NOT `bun test`.
 *
 * This registers no `bun:test` cases -- it is a script with its own `check()`
 * counter and a top-level `await main()` -- so `bun test` reports "Ran 0 tests"
 * and swallows the failure signalled through `process.exitCode`.
 *
 * It runs under node too, through esbuild for the same reason scripts/build.mjs
...
async function main(): Promise<void> {
  ...
  // `normalizeRoutes` over these routes and the `images.sizes` list were both
  // checked here and both were duplicates: vercel.ts calls normalizeRoutes and
  // throws during every build, and `sizes` is generated from the one
  // IMAGE_WIDTHS constant vercel.ts imports, so no second transcription exists
  // to drift.
