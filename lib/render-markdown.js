...
const footnoteTemplate = (ref, text) => {
  return `<small id="${footnotePrefix}:${ref}">${ref}</small>${text}`;
};
const referenceTemplate = ref => {
  return renderToStaticMarkup(
    <sup id={`${referencePrefix}:${ref}`}><a href={`#${footnotePrefix}:${ref}`} as={`#${footnotePrefix}:${ref}`} className={linkStyles.transition}>
     {ref}
    </a></sup>
  )
