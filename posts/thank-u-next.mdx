...
- It's more of a build script. `build.ts` reads `posts/`, renders every route to a static index.html with React (on the server), and writes a [Vercel Build Output](https://vercel.com/docs/build-output-api) directory. Build time is ~half a second warm on my M2 MBP.
