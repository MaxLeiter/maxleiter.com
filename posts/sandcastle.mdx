...
to [run on a jailbroken iPad](https://maxleiter.com/blog/X11). I wanted to run real applications on my iPad.
...
I built this on Vercel Sandbox, but any sandbox provider or Linux machine can host the same. I hope to package this into a
single binary that can be run on any Linux host, but right now the code is Vercel Sandbox specific.

There's a bunch to love about sandboxes, but what I love is their ephemeral nature and ability to scale.
I can spin up as many sandboxes as I want, wallet permitting.

I also love running agents (_also_ wallet permitting). But I hate having to manually approve their `npm` commands or reviewing the scripts
...
But if agents are going to write and run code, install packages, and manipulate browsers, maybe the interface to that shouldn't be a terminal &mdash; _anything_ running on your own machine.
